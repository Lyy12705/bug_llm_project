...
def evaluate_records(gold_rows: list[dict[str, Any]], pred_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    metrics["per_repo"] = _per_repo_summary(pairs)
...
def _per_repo_summary(pairs: list[tuple[dict[str, Any], dict[str, Any]]]) -> list[dict[str, Any]]:
    grouped: dict[str, list[tuple[dict[str, Any], dict[str, Any]]]] = {}
    for gold, pred in pairs:
        repo = _repo_value(gold) or _repo_value(pred) or "unknown"
        grouped.setdefault(repo, []).append((gold, pred))
    ...
    for repo in sorted(grouped):
        ...
        for gold, pred in rows:
            ...
            if not gold_files:
                continue
            evaluated += 1
            rank = _first_relevant_file_rank(_candidate_files(pred), gold_files)
            if rank is None:
                continue
        ...
        summary.append(
            {
                "repo": repo,
                "rows": len(rows),
                "rows_with_file_ground_truth": evaluated,
                "file_top_1_accuracy": _ratio(hits_at[1], evaluated),
                "file_top_3_accuracy": _ratio(hits_at[3], evaluated),
                "file_top_5_accuracy": _ratio(hits_at[5], evaluated),
                "file_mrr": _ratio(reciprocal_rank_total, evaluated),
            }
        )
    return summary
